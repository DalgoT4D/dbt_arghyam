...
WITH extract_data AS (
    SELECT
        ...
        CAST(
            ...
            ->> 'How many members attended the meeting' AS INT
        ) AS num_members_attended,
        CAST(
            CAST(observations AS JSONB)
            ->> 'How many women participants attended the meeting' AS INT
...
SELECT
    ...
    exd.meeting_date,
    EXTRACT(MONTH FROM CAST(meeting_date AS TIMESTAMP)) AS reporting_month,
    EXTRACT(YEAR FROM CAST(meeting_date AS TIMESTAMP)) AS reporting_year,
    exd.username,
    ...
    brd.district_name,
    brd.gp_name,
    CAST(created_at_timestamp AS TIMESTAMP),
...
FROM
    extract_data AS exd
LEFT JOIN
    {{ ref('activity_dim') }} AS activity
    ON exd.encounter_type = activity.activity_type
