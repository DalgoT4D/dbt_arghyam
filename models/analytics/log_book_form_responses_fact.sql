...
extract_fields AS (
    SELECT
        ...
        json_extract_path_text(
            raw_data.observations::json, 'Reporting Year'
        ) AS reporting_year,
        json_extract_path_text(
            raw_data.observations::json, 'Reporting month'
        ) AS reporting_month,
        json_extract_path_text(
            raw_data.observations::json,
            'Photo of the log-book of the entire month'
        ) AS photo_logbook,
        json_extract_path_text(
            raw_data.observations::json,
            'What were the reasons for not supplying water'
        ) AS reasons_no_water,
        json_extract_path_text(
            raw_data.observations::json,
            'For how many days was water not supplied in the ward'
        ) AS days_no_water,
        json_extract_path_text(
        ...
        *,
        CASE
            ...
            WHEN
                reporting_month = 'Feb'
                AND (
                    reporting_year::int % 4 = 0
                    AND (
                        reporting_year::int % 100 != 0
                        OR reporting_year::int % 400 = 0
                    )
                )
                THEN 29
...
final_calculation AS (
    SELECT
        ...
        total_days_in_month,
        total_days_in_month
        - coalesce(days_no_water::int, 0) AS days_with_water,
        CASE
            WHEN
                total_days_in_month - coalesce(days_no_water::int, 0) >= 27
                THEN 'Yes'
            ELSE 'No'
