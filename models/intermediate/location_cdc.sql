...
WITH extracted_location_data AS (
    SELECT
        ...
        to_timestamp(
            json_extract_path_text(sub.audit::json, 'Created at'),
            'YYYY-MM-DD"T"HH24:MI:SS.US"T"TZ'
        ) AS src_created_timestamp,
        to_timestamp(
            sub.last_modified_at, 'YYYY-MM-DD"T"HH24:MI:SS.US"T"TZ'
        ) AS last_modified_at
    FROM {{ source('source_arghyam_surveys', 'subjects') }} AS sub

    {% if is_incremental() %}
    WHERE TO_TIMESTAMP(sub.last_modified_at, 'YYYY-MM-DD"T"HH24:MI:SS.US"T"TZ') >= 
          (SELECT MAX(src_last_modified_at) FROM {{ this }})
    {% endif %}
),
...
SELECT
    {{ dbt_utils.generate_surrogate_key(
        ['ward_name', 'block_name', 'district_name', 'gp_name']
    ) }} AS location_id,
    ward_name,
    block_name,
    district_name,
